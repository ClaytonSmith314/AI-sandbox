...
# TODO: what happens if we combine executer and interpreter
class Executer:
    def __init__(self, interpretor, word):
        self.word = word
        self.interpretor = interpretor
        if word in heap:
            self.script = heap[word]
        else:
            if word in native:  # use a native method call to execute word
                self.script = native[word]
        self.pointer = 0
        self.status = 'active'
    ...
    def execute(self):
        if self.word in heap:
            ...
            if self.word in native:  # native words have their own functions
                return native[self.word].send(self.interpretor)  # we can run the function and directly get the result
...
def nextword():
    yield 'done', 'next'  # next suspends the parent opperation, but it itself does not suspend
